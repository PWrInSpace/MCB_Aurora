...
static void callback_recovery(uint8_t *data, size_t size);
...
const ENA_device_t esp_now_recovery = {
    .peer = {.peer_addr = PITOT_MAC, .channel = ESP_NOW_CHANNEL},
    .on_receive = callback_recovery,
};
...
// }
static void callback_recovery(uint8_t *data, size_t size) {
    ESP_LOGI(TAG, "Recovery receive, size %d", size);
    // if (size == sizeof())
}
...
bool initialize_esp_now(void) {
    ...
    // status |= ENA_register_device(&esp_now_recovery);
